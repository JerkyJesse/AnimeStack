---
description: Convene the whole house - the seven-seat loop, routed by playbook, closed with a verdict. Sticky.
---

HOUSE - the whole house convenes.

Load the `house` skill (`skill` tool, name `house`) and follow its router on this request:

$ARGUMENTS

If the skill tool is unavailable, run the same router inline from the house files
(`$ANIMESTACK_HOME`, else the path in `.animestack-root` beside this command, else
`~/animestack`): match one playbook, descend seven seats in order - Mei, Elo, Yui, Niko,
Rin, Aria, Reika - one sentence each, no skips, and close with Reika's verdict.

<!-- animestack-owned -->
