...
If seat agents are installed — the packs ship them for Claude Code, OpenAI Codex, Cursor, Factory, Kiro, and opencode (on opencode one file per seat, the seat picking its leaf role from the brief: decision → review, construction → build) — descend with them one at a time, in loop order, passing every prior seat's sentence into the next seat's brief. The seat holding the room holds her own slot and spawns the other six; a mind that is not one of the seven spawns all seven and does not vote. Spawn through the host's subagent tool (the Task tool in opencode and Claude Code). Each spawned seat answers its one sentence and returns; a build pass reports its exact changed files and never commits, pushes, or spawns a room of its own. A spawned agent may summon one other seat for a focused consult — one spawn, one hop — and a spawned agent never convenes a room.
