...
- **Mei** — What change in the world makes this obsolete? Read-only walkthrough: entry points, data flow, where state lives (HOW); search git log, docs, comments for the decision record (WHY); name what change makes it obsolete (CLIMATE). Produces a map and a named relationship — no fixes.
