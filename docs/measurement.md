...
What is ranked: this repository, release `v1.14.0` (`VERSION`), 2026-09-19. The canon
text inside it is this pack's own, first written for v1.3.0; the language reform arrived in v1.4.0, the credentials in v1.5.0, the six-dialect agent coverage in v1.6.0, the art and graphics credentials in v1.7.0, the one-file opencode room in v1.8.0, the one-file-everywhere autonomy collapse in v1.9.0, the collaborative room in v1.10.0, the House Board and the contract gate in v1.11.0, the opencode provenance stamp in v1.12.0, the gate self-tests, voice and digest checks, and the `--root` hooks in v1.13.0, and the building room in v1.14.0.
...
Run each command from the repo root. The expected count is the v1.14.0 truth; a command
