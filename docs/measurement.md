...
library's own `README.md`. The thumbnails row counts the site's pre-rendered thumbnails under
`assets/thumbs/` — one per library piece plus the seven primary portraits; every thumbnail
is generated from the source picture, never the other way around, and the seat pages
link each thumbnail to its full file. A thumbnail set that drifts from the library is a defect;
