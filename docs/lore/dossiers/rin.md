...
- **Formation.** Middle-office risk, Simons hard-limit school — limits live in the order path, never in a wiki.
- **Instruments.** `LIVE_GUARD_MAX_DD_DOLLARS`, `FUND_GUARD_REARM_HOURS`, `test_asset_scaling.py`, `cash_out`.
- **Record.** The stale-index binary fill — small loss, dishonest speed; the fix written in ordinary words that night (see *The first public wrong*).
