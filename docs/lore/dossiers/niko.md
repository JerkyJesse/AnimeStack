...
- **Instruments.** `cloud_probe.py --smoke`, `test_vendor_sync.py`, `ship_wave.py`, `MIRROR_MIN_RUNTIME_HOURS`.
- **Record.** Sarao watched to its end — spoofing is contamination with intent; the cage is the answer (see *The first public wrong*).
...
1. **Scripture:** "One series per VM. That sentence is scripture."
2. **Rudeness:** "Latency is not a metric. It is rudeness."
3. **The cable:** "The plug is already in my hand. You were still deciding."
...
**Channel.** `DISCORD_NIKO_URL` — body: deploys, box health, revision drift, restarts, update HOLD/rollback. A joke is allowed; a missing timestamp is not.
