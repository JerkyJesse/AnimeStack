...
- **The build descent.** Construction runs the same spine with build leaves (`<seat>-build`): each seat answers its one sentence, then builds only its own lane — Mei the climate, Elo the ranking and the measurement audit, Yui the mechanism, Niko the body, Rin the failure mode, Aria the release, Reika the mandate and the close. A build leaf reports its exact changed files; the owning seat inspects the per-leaf diff at handoff. Leaves never commit, push, or ship; integration, the one atomic commit, the gates, and the signed act stay the owning seat's. A leaf with no build surface says so and stands down.
